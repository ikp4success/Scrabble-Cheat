...
//		 Log.v("heEND", value_end+"end");
	...
	public class progression extends AsyncTask<Void, Void, Void> {
		ProgressDialog dialog = new ProgressDialog(Display_activity.this);
		...
		protected Void doInBackground(Void... params) throws NullPointerException {
			...
			try {
				...
				try {
					Document doc = Jsoup
							.connect("http://www.scrabblefinder.com/words-with-friends-cheat/")
					...
					word02 = doc.select("p:contains(2 Letter Words with Friends Words)")
							.text();
					word02a = doc.select(
...
//					word01a = doc.select("p:contains(1 Letter) + ul a").text();
					...
					//e.printStackTrace();
					display_tv.setText("Sorry Server is Down, try again later");
				}
